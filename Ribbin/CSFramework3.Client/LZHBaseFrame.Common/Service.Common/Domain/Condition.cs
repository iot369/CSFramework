    ...
    public class PagerCondition : Condition
    {
        ...
        private void BuildPagerSql(string sql, int inclusive, int exclusive, bool group)
        {
            if (!group)
            {
                ...
                this.SQLText = string.Format("select * from (select ROW_NUMBER() OVER (order by usercode) as rid,{0}) as a where rid between {1} and {2}",
                    this.SQLText,
                    inclusive.ToString(),
                    exclusive.ToString());
                //this.SQLText = string.Format("select * from (select rownum as rid,{0}) as a where rid between {1} and {2}",
                //    this.SQLText,
                //    inclusive.ToString(),
                //    exclusive.ToString());
            }
        ...
        private void BuildPagerSql(string sql, string orderbyFields, int inclusive, int exclusive, bool group)
        {
            ...
            if (!group)
            {
                ...
                this.SQLText = string.Format(@" select * from ( select ROW_NUMBER() OVER (ORDER BY {0}) as rid,* from (select {1}) as a ) as b where rid between {2} and {3} order by {0}",
                                            orderbyFields,
                                            this.SQLText,
                                            inclusive.ToString(),
                                            exclusive.ToString(),
            ...
            {
                this.SQLText = string.Format(@"select * from (select /*+ leading({4}) */  ROW_NUMBER() OVER (ORDER BY {0}) as rid, result.* from ({1}) result) as a where rid between {2} and {3} order by {0}",
                                            orderbyFields,
                                            sql,
                                            inclusive.ToString(),
                                            exclusive.ToString(),
