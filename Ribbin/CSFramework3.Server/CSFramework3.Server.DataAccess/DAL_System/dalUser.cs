...
namespace LZHBaseFrame.Server.DataAccess.DAL_System
{
    ...
    public class dalUser : dalBaseDataDict, IBridge_User
    {
        ...
        public DataTable GetUserDirect(string account, string DBName)
        {
            ...
            SqlCommandBase cmd = SqlBuilder.BuildSqlCommandBase(sql);
            cmd.AddParam("@Account", SqlDbType.VarChar, account);
            DataTable dt = DataProvider.Instance.GetCusDataProvider(DBName).ConditionQuery(new Core.Domain.SQLCondition("select * from [tb_MyUser] where [Account]='" + account + "'")).Tables[0];
